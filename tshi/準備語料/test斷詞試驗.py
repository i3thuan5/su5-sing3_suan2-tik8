...
class 斷詞試驗(TestCase):
    ...
    def test_有對應詞性(self, 斷詞mock):
        斷詞mock.return_value = [['我', 'N'], ['愛', 'V'], ['小豬', 'N']]
        語料 = 準備語料().斷詞(
            拆文分析器.建立句物件('我 愛 小豬',)
        )
        self.assertEqual(語料['國教院對應詞性'], ['N', 'V', 'N'])

    @patch('臺灣言語工具.斷詞.國教院斷詞用戶端.國教院斷詞用戶端.語句斷詞做陣列')
    def test_敆做伙時算後一字(self, 斷詞mock):
        斷詞mock.return_value = [['我', 'N'], ['愛', 'V'], ['小豬', 'N']]
        語料 = 準備語料().斷詞(
            拆文分析器.建立句物件('我 愛 小 豬',)
        )
        self.assertEqual(語料['國教院對應詞性'], ['N', 'V', 'N', 'N'])

    @patch('臺灣言語工具.斷詞.國教院斷詞用戶端.國教院斷詞用戶端.語句斷詞做陣列')
    def test_分開時算後一字(self, 斷詞mock):
        斷詞mock.return_value = [['我', 'N'], ['愛', 'V'], ['小', 'A'], ['豬', 'N']]
        語料 = 準備語料().斷詞(
            拆文分析器.建立句物件('我 愛 小豬',)
        )
        self.assertEqual(語料['國教院對應詞性'], ['N', 'V', 'N'])
